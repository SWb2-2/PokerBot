...
const arr_suit =   [ undefined, 
                     undefined, 
                    [undefined, undefined, 00000, 38.28, 39.06, 40.00, 39.72, 40.07, 42.07, 44.48, 47.11, 49.77, 52.74, 55.76, 59.84],
                    [undefined, undefined, 38.28, 00000, 40.75, 41.79, 41.47, 41.94, 42.68, 45.29, 47.91, 50.58, 53.46, 56.55, 60.58],
                    [undefined, undefined, 39.06, 40.75, 00000, 43.45, 43.12, 43.64, 44.28, 45.80, 48.71, 51.33, 53.28, 57.29, 61.42],
                    [undefined, undefined, 40.00, 41.79, 43.45, 00000, 44.83, 45.37, 46.12, 47.49, 49.31, 52.10, 54.98, 58.15, 62.13], 
                    [undefined, undefined, 39.72, 41.47, 43.23, 44.83, 00000, 46.73, 47.68, 49.04, 50.74, 52.64, 55.73, 58.79, 62.02], 
                    [undefined, undefined, 40.07, 41.94, 43.64, 45.47, 46.73, 00000, 49.13, 50.55, 52.23, 54.06, 56.17, 59.59, 62.88], 
                    [undefined, undefined, 42.07, 42.68, 44.28, 46.12, 47.68, 49.12, 00000, 51.96, 53.85, 55.67, 57.75, 60.16, 63.69], 
                    [undefined, undefined, 44.48, 45.29, 45.80, 47.49, 49.04, 50.55, 51.96, 00000, 55.59, 57.42, 59.47, 61.89, 64.62], 
                    [undefined, undefined, 47.11, 47.91, 48.71, 49.31, 50.74, 52.23, 53.85, 55.59, 00000, 59.38, 61.45, 63.69, 66.51], 
                    [undefined, undefined, 49.77, 50.58, 51.33, 52.10, 52.64, 54.06, 55.67, 57.42, 59.38, 00000, 62.30, 64.61, 67.34],
                    [undefined, undefined, 52.74, 53.46, 53.28, 54.98, 55.73, 56.27, 57.75, 59.47, 61.45, 62.30, 00000, 65.52, 68.16], 
                    [undefined, undefined, 55.75, 56.55, 57.29, 58.15, 58.79, 59.59, 60.16, 61.89, 63.69, 64.61, 65.52, 00000, 69.24],
                    [undefined, undefined, 59.84, 60.58, 61.42, 62.13, 62.02, 62.88, 63.69, 64.62, 66.51, 67.34, 68.16, 69.24, 00000]
                    ]
...
let result = equity_range([new Card(14,3), new Card(14,2)]);
console.log(result);
...
//         let current_winner = equity([new Card(i, 0), new Card(k, 0)]);
//         if(k == i) {
...
//         let current_winner = equity([new Card(i, 0), new Card(k, 1)]);
//         if(k == i) {
//             result_pair.push({rank: i, w_r: current_winner.draw_and_winrate});
//         } else {
...
//With range
function equity_range(hand, table, range_low, range_high) {
    if(table == undefined) {
        table = [];
    }
    ...
    for(let i = 0; i < 10000; i++) {
        ...
        //Make sure the oppent get cards in the given range. 
        let flag = 1; 
