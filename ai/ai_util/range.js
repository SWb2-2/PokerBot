...
function determine_range(data, player_move, pot_size, first) {
    //console.log.log("Current range", data.current_range); 
    ...
    if(player_move.move == undefined) {
        ...
        // //console.log.log("Ra is ", ra, " Pot is ", pot_size, " amount is ", player_move.amount);
        let w_cr, w_hp,  w_c;
        let i_cr, i_hp, i_c;
        ...
        if(player_move.move == "check") {
            ...
            // //console.log.log("new range after check", data.current_range);
            return data.current_range;

        } else if(player_move.move == "call") {
            ////console.log.log("C is ", c," Cr is ", cr, " Hp is ", hp, data.current_range);
            if(first) {
                data.current_range.range_Low = 40;
                data.current_range.range_high = 86;
            }
            ...
            ////console.log.log("Before ", data.current_range);
            range_control_call(data.current_range);
            ////console.log.log("After ", data.current_range);
            // //console.log.log("new range after call", data.current_range);
            return data.current_range; 
            ...
            // //console.log.log("change in range", (Math.pow(10, 3.5 - st)/data.current_range.range_Low) - (data.current_range.range_Low/10)); 
            data.current_range.range_Low += (Math.pow(10, 3.5 - st)/data.current_range.range_Low) - (data.current_range.range_Low/10);
            //console.log.log("range_low", data.current_range.range_Low);
            // let factor = ra; 
            // if(ra > 1) {
            ...
            // //console.log.log("Your range is: ", data.current_range.range_Low, " After a bet of", player_move.amount);
            data.current_range.range_high = data.current_range.range_Low + 30;
            range_control_raise(data.current_range);
            // //console.log.log("new range after raise", data.current_range);
            return data.current_range; 
        }
    }
    return data.range;
...
function range_control_check(current_range) {
    ...
    if(current_range.range_high - 3 < current_range.range_Low) {
        // //console.log.log("Error in range_control check 1", current_range); 
    }
    ...
    if(current_range.range_high - 3 < current_range.range_Low) {
        ...
        // //console.log.log("Error in, range_control check 2", current_range); 
    }
}
...
function range_control_call(current_range) {
    if(current_range.range_high - 3 < current_range.range_Low) {
        // //console.log.log("Error in range_control call 1", current_range); 
    }
    ...
    if(current_range.range_high - 3 < current_range.range_Low) {
        // //console.log.log("Error in, range_control call 2", current_range); 
    }
}
...
function range_control_raise(current_range) {
    if(current_range.range_high - 3 < current_range.range_Low) {
        // //console.log.log("Error in, range_control raise 1", current_range); 
    }
